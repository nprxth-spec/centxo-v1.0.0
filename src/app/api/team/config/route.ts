...
export async function GET(req: NextRequest) {
  try {
    ...
    // Fallback: If no team members, use MetaAccount (from Facebook login) to fetch data
    if (teamMembers.length === 0) {
      ...
      let fbName: string | null = session.user.name || null;
      ...
      let metaUserId = metaAccount?.metaUserId || null;
      if (!token) {
      ...
      if (token) {
        teamMembers = [{
          ...
          facebookName: fbName || session.user.email || 'Facebook User',
