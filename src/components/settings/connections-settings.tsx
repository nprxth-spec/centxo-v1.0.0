    ...
    return (
        <div className="space-y-6">
            ...
            <div className="space-y-5">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">
                                {facebookMembers.length || (mainFacebookProfile ? 1 : 0)} {(facebookMembers.length || (mainFacebookProfile ? 1 : 0)) === 1 ? 'account' : 'accounts'}
                            </p>
                        </div>
                    </div>
                    <Button
                ...
                {facebookMembers.length === 0 && !mainFacebookProfile ? (
                    ...
                    <div className="space-y-1.5">
                        <div className="group h-[60px] flex items-center gap-3 px-4 py-2 rounded-lg border border-gray-200 bg-green-50/30 border-green-200">
                            {mainFacebookProfile.image ? (
                                <div className="w-10 h-10 rounded-lg flex-shrink-0 shadow-sm overflow-hidden">
                                    <img src={mainFacebookProfile.image} alt={mainFacebookProfile.name} className="w-full h-full object-cover" />
                                </div>
                            ) : (
                                <div className="w-10 h-10 rounded-lg flex-shrink-0 shadow-sm bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-white text-sm font-bold">
                                    {mainFacebookProfile.name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)}
                                </div>
                            )}
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                    <span className="text-sm font-semibold text-gray-900 truncate">{mainFacebookProfile.name}</span>
                                    <span className="px-2 py-0.5 bg-green-100 text-green-700 rounded text-xs font-medium flex-shrink-0">
                                        Connected
                                    </span>
                                </div>
                                <p className="text-xs text-gray-500 truncate mt-0.5">Logged in with Facebook</p>
                            </div>
                        </div>
                    </div>
                ) : (
