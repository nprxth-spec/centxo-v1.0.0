...
export function LandingFooter() {
    ...
    return (
        <footer className="flex flex-col gap-4 py-6 w-full shrink-0 px-4 md:px-6 border-t bg-muted/30">
            {/* Privacy Policy - Must be visible in footer for Google OAuth verification */}
            <div className="flex flex-wrap gap-4 justify-center sm:justify-between items-center">
                <p className="text-sm font-medium text-foreground">
                    <Link href={PRIVACY_URL} className="text-primary hover:underline underline-offset-4 font-semibold">
                        Privacy Policy
                    </Link>
                    {" · "}
                    <Link href={TERMS_URL} className="text-primary hover:underline underline-offset-4">
                        Terms of Service
                    </Link>
                </p>
            </div>
