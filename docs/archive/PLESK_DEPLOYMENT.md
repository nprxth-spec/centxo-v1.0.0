...
### 3. Database Connection Failed / Build ล้มเหลว
**สาเหตุ:** 
- Prisma Client ยังไม่ได้ถูก generate
- `prisma db push` ล้มเหลวเพราะ build server เชื่อมต่อ DB ไม่ได้ (firewall, env ไม่โหลด)

**วิธีแก้:** 
- ถ้า `npm run build` ล้มเหลว: ใช้ `npm run build:prod` แทน (ไม่รัน db push ตอน build)
- รัน `npx prisma db push` แยกต่างหากหลังจาก build สำเร็จ (เมื่อ env โหลดแล้ว)
